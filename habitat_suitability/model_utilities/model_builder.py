...
class ModelBuilder():
    ...
    def grid_search(self, n_estimators=[100, 200, 500], max_features=['sqrt', 'log2'], max_depth=[4, 5, 6, 7, 8],
                    criterion=['squared_error', 'absolute_error'], random_state=42, min_samples_split=[2, 5, 10], min_samples_leaf=[1, 2, 4, 10]):
        """
        ...
        if self.model_type == 'RandomForestRegressor':
            ...
            random_forest_tuning = RandomForestClassifier(random_state=random_state)
            param_grid = {
                'n_estimators': n_estimators,
                'max_features': max_features,
                'max_depth': max_depth,
    ...
    def evaluate(self, X_data, y_data, save_figures=True, saving_base_output_name='', feature_importance=False):
        ...
        if self.model_type == 'RandomForestClassifier':
            ...
            kappa = cohen_kappa_score(y_data, y_pred_test)
    ...
    def inference(self, mask_band, tiles_folder_name, tiles_in_GCP, output_merged_tif='final_merged_predictions_result.tif',
        ...
        merge(names=paths_pred_rasters, out_file=output_merged_tif, format = 'GTiff', verbose=0, nodata=0, 
              a_nodata=0, band_type="Float32", create_options=['COMPRESS=DEFLATE', 'BIGTIFF=YES']) 
